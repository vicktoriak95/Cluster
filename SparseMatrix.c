...
Node_matrix* list_create_node(double value, int col_index){
	Node_matrix* node;
...
/* Free list of a row */
void list_free_list(Node_matrix* list){
	if (list != NULL){
		list_free_list((Node_matrix*)list->next);
		free(list);
	}
}
...
void list_free(struct _spmat *A){
	...
	for(i = 0; i< A->n; i++){
		list_free_list(((Node_matrix** )A->private)[i]);
	}
	free(A->private);
	free(A);
}
...
void list_mult(const struct _spmat *A, const double *v, double *result, Node* g, int n_g){
	int i;
	double dot_product;
