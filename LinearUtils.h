...
double* Bhat_multiplication(struct Network* N, double* x);
...
double Bhat_norm(struct Network* N);

/* Calculates the eigenvalue corresponding to largest eigen_vector */
/* Shifts result by norm */
double Bhat_largest_eigenvalue(struct Network* N, double norm, double* eigen_vector);
